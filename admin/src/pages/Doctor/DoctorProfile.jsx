...
const DoctorProfile = () => {
  ...
  return docProfileData ? (
      ...
      <div className="flex flex-col gap-4 m-5 sm:flex-row">
        ...
        <div className="flex-1 border border-zinc-200 rounded-lg p-8 py-7 bg-white shadow-xs">
          ...
          <div className="flex items-center gap-3 pt-2">
            {/* Toggle Container */}
            <div
              className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors duration-300 shadow-xs ${
                !isEditing ? "cursor-not-allowed opacity-70" : "cursor-pointer"
              } ${docProfileData.available ? "bg-primary" : "bg-zinc-300"}`}
              onClick={() =>
                isEditing &&
                setDocProfileData((prev) => ({
                  ...prev,
            ...
            >
              ...
              <span
                className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform duration-300 shadow-sm ${
                  docProfileData.available ? "translate-x-6" : "translate-x-1"
                }`}
              />
            </div>
            {/* Availability Label */}
            <label
              ...
              } ${docProfileData.available ? "text-primary" : "text-zinc-500"}`}
            >
    ...
    <div className="max-w-6xl">
      <div className="flex flex-col gap-4 m-5 sm:flex-row animate-pulse">
        {/* --- Doctor Image Skeleton --- */}
        <div className="flex-shrink-0">
          <div className="w-full sm:w-64 h-64 bg-slate-200 rounded-lg"></div>
        </div>
